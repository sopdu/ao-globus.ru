...
<?#='<pre>'; print_r($item); '</pre>';?>
    <div class="cart_inner" style="background: #fff;">
            <div class="cart row">
                ...
                <div class="to-cart col-6 <?=(sopdu::inCard($item["ID"]) == 'N') ? 'shown' : ''?>" data-value="<?= $item['ADD_URL'];?>" >
                    В корзину
                </div>
                <div id="ajaxaction=delete&ajaxdeleteid=<?=sopdu::inCard($item["ID"])?>" class="to-cart delete col-6 <?=(sopdu::inCard($item["ID"]) != 'N') ? 'shown' : ''?>" data-value="?drop_item=<?=sopdu::inCard($item["ID"])?>&view=tile" style="background: #f2f2f2; color: #0A2752">
