...
<div class="<?=(!$USER->IsAuthorized()) ? 'non-autorized' : ''?> simprod_item col-sm-3">
    ...
    <a href="<?=$item['DETAIL_PAGE_URL']?>"><img src="<?=$item["DETAIL_PICTURE"]["SRC"]?>" alt="<?=$item["NAME"]?>" class="simprod_item_img"></a>
    <a href="<?=$item['DETAIL_PAGE_URL']?>"><div class="simprod_item_name text-center"><?=$item["NAME"]?></div></a>
    <??>
    <div class="simprod_item_price text-center"><?=$item["MIN_PRICE"]["VALUE"]?>₽/ед.</div>
    ...
    <div class="cart_inner" style="background: #fff;">
            <div class="cart row">
                ...
                <?if(sopdu::inCard($item["ID"]) == 'N'):?>
                    <div class="to-cart col-6" data-value="<?= $item['ADD_URL'];?>"  onclick="window.location.reload();">
                        В корзину
                    </div>
                <?else:?>
                    <a id="ajaxaction=delete&ajaxdeleteid=<?=sopdu::inCard($item["ID"])?>" class="to-cart col-6" href="?drop_item=<?=sopdu::inCard($item["ID"])?>&view=tile" onclick="window.location.reload();" style="background: #f2f2f2; color: #0A2752">
                        <?/*<a class="to-cart col-6" href="?drop_item=<?=$item["ID"]?>" style="background: #f2f2f2; color: #0A2752">*/?>
                        Удалить
                    </a>
                <?endif;?>
                <?#='<pre>'; print_r($item); '</pre>';?>
...
<?else:?>
    <div class="<?=(!$USER->IsAuthorized()) ? 'non-autorized' : ''?> simprod_item col-sm">
        <div class="simprod_item_status d-flex justify-content-betwee">
            <?if($item["PROPERTIES"]["HIT"]["VALUE_XML_ID"] == 'Y'):?>
                <div class="simprod_item_hit ">Хит</div>
            <?endif;?>
