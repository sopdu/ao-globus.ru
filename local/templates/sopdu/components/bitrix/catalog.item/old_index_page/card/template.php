...
<div class="simprod_item col-sm">
    ...
    <img src="<?=$item["DETAIL_PICTURE"]["SRC"]?>" alt="<?=$item["NAME"]?>" class="simprod_item_img">
    <div class="simprod_item_name text-center"><?=$item["NAME"]?></div>
    <div class="simprod_item_price text-center"><?=$item["PRICES"]["BASE"]["VALUE"]?>₽/ед.</div>
    <?if(!empty($prop["PACKAGE"])):?>
        <div class="samprod_kol">В упаковке <?=$prop["PACKAGE"]?>шт.</div>
    <?endif;?>
...
<?#='<pre>'; print_r($prop); '</pre>';?>
<div class="simprod_item col-sm">
    <div class="simprod_item_status d-flex justify-content-betwee" style="z-index: 1">
        <?#='<pre>'; print_r($item["IBLOCK_ID"]); '</pre>';?>
        <?#='<pre>'; print_r($item["ID"]); '</pre>';?>
        <?
        $zapros = CIBlockElement::GetProperty(
            $item["IBLOCK_ID"],
            $item["ID"],
            [],
            []
        ...
        while ($row = $zapros->Fetch()){
            if(!empty($row["VALUE"]) and $row["CODE"] == 'HIT'){
                echo '<div class="simprod_item_hit ">Хит</div>';
            }
            if(!empty($row["VALUE"]) and $row["CODE"] == 'NEW'){
                echo '<div class="simprod_item_new">Новинка</div>';
            }
        }
        ?>
    </div>
    ...
    $res = $zapros->Fetch()["VALUE"]
    ?>
    <?if(!empty($res)):?>
        <div class="samprod_kol">В упаковке <?=$res?>шт.</div>
    <?endif;?>
    <div class="cart_inner" style="background: #fff;">
        <div class="cart row">
