...
<div class="dropdown-menu_test">
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
			...
			<?endforeach;?>
										...
										<div class="smart-filter-input-group-number">
											...
											<div class="smart-filter-slider-track-container">
												<div class="smart-filter-slider-track" id="drag_track_<?=$key?>">
													<?//for($i = 0; $i <= $step_num; $i++):?>
													<div class="smart-filter-slider-ruler p1"><span><?=$prices[0]?></span></div>
													<div class="smart-filter-slider-ruler p5"><span><?=$prices[4]?></span></div>
													<?//endfor;?>
							...
							switch ($arItem["DISPLAY_TYPE"])
							{
								...
								default://CHECKBOXES
									?>
										...
										<?foreach($arItem["VALUES"] as $val => $ar):?>
											<li class="subitem">
												<input class="subitem_checkbox" type="checkbox" value="<? echo $ar["HTML_VALUE"] ?>" name="<? echo $ar["CONTROL_NAME"] ?>" id="<? echo $ar["CONTROL_ID"] ?>" <? echo $ar["CHECKED"]? 'checked="checked"': '' ?> onchange="smartFilter.click(this)">
				...
				?>
                <?
                    foreach ($arResult["COMBO"][0] as $comb){
                        if(!empty($comb)){
                            $combo = 'Y';
                        }
                    }
                foreach ($arResult["COMBO"][1] as $comb){
                    if(!empty($comb)){
                        $combo = 'Y';
                    }
                }
                ?>
                <?#='<pre>'; print_r($combo); '</pre>';?>
                <?#='<pre>'; print_r($arResult["COMBO"]); '</pre>';?>
			...
			<li class="item_test">	
					...
					<div class="bx-filter-block">
						<div class="bx-filter-parameters-box-container">
							...
							<div class="bx-filter-popup-result <?if ($arParams["FILTER_VIEW_MODE"] == "VERTICAL") echo $arParams["POPUP_POSITION"]?>" id="modef" <?if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"';?> style="display: inline-block;">
								<?echo GetMessage("CT_BCSF_FILTER_COUNT", array("#ELEMENT_COUNT#" => '<span class="d-none" id="modef_num">'.intval($arResult["ELEMENT_COUNT"]).'</span>'));?>
								<span class="arrow"></span>
								
								<a class="btn btn-themes" id="set_filter" href="<?echo $arResult["FILTER_URL"]?>" target=""><?echo GetMessage("CT_BCSF_FILTER_SHOW")?></a>
							</div>
						</div>
					</div>
				</ul>
                ...
                <?else:?>
                    <script>$('#openfilters').hide()</script>
			<?endif;?>
			<div class="clb"></div>
		</ul>
		</form>
