...
<a href="/personal/cart/"  id="<?=$cartId?>">
    <span class="basket_num">
        <img src="<?=SITE_TEMPLATE_PATH?>/img/basket.svg">
        <span class="numbask"><?=$arResult["NUM_PRODUCTS"]?></span>
    </span>
    <?
        $price = explode(' ', $arResult["TOTAL_PRICE"]);
        array_pop($price);
    ?>
    <span class=" mr-3 bal" >
        <?
            foreach ($price as $pr){ echo $pr; }
        ?>
        <span id="simvolfix">₽</span>
    </span>
</a>
